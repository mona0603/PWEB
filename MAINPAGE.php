...
<body>
    ...
    <header class="header">
        ...
        <nav class="navbar">
            ...
            <?php if ($auth['isAdmin']): ?>
                <i class="fa-solid fa-crown admin-crown" title="Administrador"></i>
    ...
    <section class="main">
        ...
        <section class="page-content">
            <!-- sliders -->
            <div class="slider">
                <?php if (!empty($newsSlides)): ?>
                    <div class="slides">
                        <?php foreach ($newsSlides as $index => $news): ?>
                            <div class="slide <?= $index === 0 ? 'active' : '' ?>" data-postid="<?= $news['PostID'] ?>">
                                <?php if (str_starts_with($news['MediaType'], 'image/')): ?>
                                    <img src="get_post_media.php?id=<?= $news['PostID'] ?>" alt="News">
                                <?php elseif (str_starts_with($news['MediaType'], 'video/')): ?>
                                    <video autoplay muted loop>
                                        <source src="get_post_media.php?id=<?= $news['PostID'] ?>" type="<?= $news['MediaType'] ?>">
                                    </video>
                                <?php endif; ?>
                                ...
                                <div class="info">
                                    ...
                                    <button class="news-more-info-btn">More info</button>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="navigation-manual">
                        <?php foreach ($newsSlides as $index => $news): ?>
                            <div class="manual-btn <?= $index === 0 ? 'active' : '' ?>"></div>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <!-- <div class="no-news">
                    ...
                    <img src="img/sliderbg.png" alt="slider" class="sliderbg">
                    <div class="info">
                        <h1>No news available at the moment.</h1>
                        <p>Welcome to FIFA, there's no current news available at the moment</p>
                    </div>
                <?php endif; ?>
            </div>
                ...
                <div class="post-container">
                    <div class="sort-by">
                        ...
                        <select name="sort" id="sort-select">
                            ...
                            <!-- AQUI FALTA EL COUNTRY PERO AUN NO LO TENGO -->
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        $stmtProfile->bind_param("ii", $loggedInId, $postUserId);
                        $stmtProfile->execute();
                        $displayProfile = $stmtProfile->get_result()->fetch_assoc();

                        ?>

                        <div class="post-container-aux secpost" data-postid="<?= $row['PostID'] ?>">
                            <a class="post" href="POST.php?id=<?= $row['PostID'] ?>">
                                    ...
                                    <div class="card-dropdown">
                                        ...
                                        <div class="dropdown-profile">
                                            ...
                                            <div class="dd-info">
                                                ...
                                                <p class="dd-username">@<?= htmlspecialchars($displayProfile['Username']) ?>
                                                </p>
                                ...
                                <div class="card-texta">
                                    <input type="text" readonly value="<?= htmlspecialchars($row['Title']) ?>"
                                        class="card-title">
                                    <textarea readonly maxlength="360"><?= htmlspecialchars($row['Content']) ?></textarea>
                                ...
                                <?php if (!empty($row['Media']) && !empty($row['MediaType'])): ?>
                                    <div class="card-media">
                                        <?php if (str_starts_with($row['MediaType'], 'image/')): ?>
                                            <img src="get_post_media.php?id=<?= $row['PostID'] ?>" alt="media"
                                ...
                                <div class="card-btns">
                                    <button class="like-btn" data-postid="<?= $row['PostID'] ?>"
                                        ...
                                        <i class="<?= $row['hasLiked'] ? 'fa-solid liked' : 'fa-regular' ?> fa-star"></i>
                            ...
                            <div class="c-opt">
                                ...
                                <div class="c-menu">
                                    <?php if ($loggedInId === $postUserId): ?>
                                        <!-- Editar (solo autor) -->
                                        <button type="button" class="edit-post-btn iconi" data-postid="<?= $row['PostID'] ?>">
                                    ...
                                    <?php if ($loggedInId !== $postUserId && !$isAdmin): ?>
                                        <button type="button" class="report-post-btn iconi" data-postid="<?= $row['PostID'] ?>">
    ...
    <div class="modal"> <!-- fondo -->
        <div class="modal-content" id="modal-login"> <!-- ventana base -->
            ...
            <a href="google_login.php" class="m-opt" style="text-decoration:none; color:inherit;">
